...
from Navigation_function import *
from TTS import TTS
from STT import STT
from config import *
from key_local import *
from key import *
...
while True:
    ...
    if poi_response.status_code == 200:
        ...
        #TTS(f"검색 결과, 총 {poi_count}개의 장소가 검색되었습니다.", "총 몇 개의 장소가 검색되었습니다.mp3")

        if poi_count == 1:#검색 결과가 1개일 때
            while True:
                TTS(f"{name_output[0]}으로 안내할까요?", "is_this_place.mp3")#name_output에 있는 첫 번째 name을 말함
                choice = STT()
                ...
                #응답이 No일 때 -> break -> else문으로 이동-> 장소 검색을 다시 함)
                    ...
                    choice_name = ""
                    TTS("다른 검색 결과가 없습니다. 목적지를 다시 말해주세요.", "retry_location.mp3")
                    break
                
                elif any(response in choice.lower() for response in retry_responses):
                    TTS("목적지를 다시 말해주세요.", "research.mp3")
                    choice = "oh_my_god_retry"
                    choice_name = ""
                    break

                else:
                    TTS(f"응답을 이해하지 못했습니다.", "icant.mp3")
                    choice =""
                    continue  
        ...
        if poi_count == 2:
            while True:
                TTS(f"{name_output[0]}으로 안내할까요?", "isthisplace_1.mp3")#name_output에 있는 첫 번째 name을 말함
                choice = STT()
                ...
                #응답이 no일 때는 다음 장소로 안내하도록 함
                    ...
                    TTS(f"{name_output[1]}으로 안내할까요?", "isthisplace1.mp3")
                    choice = ""
                    choice = STT()
                    ...
                    #
                        ...
                        TTS("다른 검색 결과가 없습니다. 목적지를 다시 말해주세요.", "retry.mp3")
                        break

                else:
                    TTS("응답을 이해하지 못했습니다. 다시 말해 주세요.", "idon'tknow.mp3")
                    choice = STT()
                    continue

        if poi_count == 3:
            while True:
                TTS(f"{name_output[0]}으로 안내할까요?", "isthisplace1.mp3")#name_output에 있는 첫 번째 name을 말함
                choice = STT()
                ...
                if any(response in choice.lower() for response in positive_responses):
                    ...
                    if any(response in choice.lower() for response in positive_responses):
                        ...
                        TTS(f"{name_output[2]}으로 안내할까요?", "isthisplace1.mp3")
                        choice = ""
                        choice = STT()
                        if any(response in choice.lower() for response in positive_responses):
                    ...
                    TTS("죄송합니다. 응답을 이해하지 못했습니다. 다시 실행해 주세요.", "isthisplace1.mp3")
                    choice = STT()
                    continue    

        if choice_name:
            TTS(f"{choice_name}으로 안내를 시작합니다", "isthisplace1.mp3")
    ...
    for x in range(len(description_list)):#안내점 리스트 요소의 개수만큼 반복
        ...
        while True:
            ...
            if now_lat is None or now_lon is None:#GPS 좌료를 인식 할 수 없을 때
                ...
                TTS("GPS 신호를 찾을 수 없습니다.", "GPSnotfound")
                time.sleep(1)
                continue
                ...
                if distance_to_target <= next_announcement_distance:
                    TTS(f"다음 안내점까지 {int(distance_to_target)}미터 남았습니다.", "next_point")
                    # 다음 알림 거리 설정 (30m 간격)
